-- Milestone 1 (Supabase): profiles extending auth.users.
-- Supabase Auth owns identity, email verification (OTP), and sessions.
-- We store only app-specific fields here, keyed to auth.users.
...
-- security definer so the auth admin role can write into public.
